...
def create_embedding_provider(
    config: EmbeddingConfig, verbose: bool = False
) -> BaseEmbedding:
    ...
    if not provider_name:
        ...
        error(
            f"Unknown embedding config type: {config_type.__name__}. Available: {available}"
        )
...
def create_llm_provider(config: LLMConfig, verbose: bool = False) -> LLM:
    ...
    if not provider_name:
        available = list(provider_map.values())
        error(
            f"Unknown LLM config type: {config_type.__name__}. Available: {available}"
        )
