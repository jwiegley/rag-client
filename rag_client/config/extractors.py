...
class EntityExtractorConfig(ModelConfig):
    ...
    model_name: str = Field(
        default="gpt-3.5-turbo", description="LLM model for entity extraction"
    )
    ...
    include_confidence: bool = Field(
        default=True, description="Whether to include confidence scores"
    )
    min_confidence: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Minimum confidence threshold"
    )
    context_window: int = Field(
        default=500, gt=0, le=2000, description="Context window around entity mentions"
    )
    deduplicate: bool = Field(
        default=True, description="Whether to deduplicate entities"
    )
...
class KeywordExtractorConfig(ModelConfig):
    ...
    model_name: str = Field(
        default="gpt-3.5-turbo", description="LLM model for keyword extraction"
    )
    ...
    extraction_method: Literal["llm", "tfidf", "rake", "yake", "keybert"] = Field(
        default="llm", description="Keyword extraction method"
    )
    include_scores: bool = Field(
        default=True, description="Whether to include relevance scores"
    )
    min_keyword_length: int = Field(
        default=3, ge=1, le=50, description="Minimum keyword length in characters"
    )
    max_keyword_length: int = Field(
        default=30, ge=1, le=100, description="Maximum keyword length in characters"
    )
    ngram_range: tuple[int, int] = Field(
        default=(1, 3), description="N-gram range for multi-word keywords"
    )
    remove_stopwords: bool = Field(
        default=True, description="Whether to remove stopwords"
    )
    language: str = Field(
    ...
    diversity: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Diversity of keywords (for KeyBERT)"
    )
    ...
    def validate_keyword_lengths(cls, v: int, info) -> int:
        ...
        if "min_keyword_length" in info.data and v <= info.data["min_keyword_length"]:
            raise ValueError(
                "max_keyword_length must be greater than min_keyword_length"
            )
        return v
...
class SummaryExtractorConfig(ModelConfig):
    ...
    model_name: str = Field(
        default="gpt-3.5-turbo", description="LLM model for summarization"
    )
    summary_length: int = Field(
        default=150, ge=20, le=1000, description="Target summary length in words"
    )
    summary_type: Literal["abstractive", "extractive", "hybrid"] = Field(
        default="abstractive", description="Type of summarization"
    )
    ...
    preserve_key_phrases: bool = Field(
        default=True, description="Whether to preserve important phrases"
    )
    include_metadata: bool = Field(
        default=False, description="Whether to include document metadata"
    )
    ...
    chunk_summaries: bool = Field(
        default=False, description="Whether to summarize chunks separately first"
    )
...
class QAExtractorConfig(ModelConfig):
    ...
    model_name: str = Field(
        default="gpt-3.5-turbo", description="LLM model for QA generation"
    )
    ...
    answer_style: Literal["brief", "detailed", "academic"] = Field(
        default="brief", description="Style of answers"
    )
    max_answer_length: int = Field(
        default=100, ge=10, le=500, description="Maximum answer length in words"
    )
    ...
    difficulty_level: Literal["basic", "intermediate", "advanced"] = Field(
        default="intermediate", description="Difficulty level of questions"
    )
    ...
    min_confidence: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Minimum confidence for QA pairs"
    )
...
class MetadataExtractorConfig(BaseConfig):
    ...
    extract_title: bool = Field(
        default=True, description="Whether to extract document title"
    )
    extract_author: bool = Field(
        default=True, description="Whether to extract author information"
    )
    extract_date: bool = Field(default=True, description="Whether to extract dates")
    extract_language: bool = Field(
        default=True, description="Whether to detect language"
    )
    extract_categories: bool = Field(
        default=True, description="Whether to extract categories/tags"
    )
    ...
    confidence_threshold: float = Field(
        default=0.8, ge=0.0, le=1.0, description="Confidence threshold for extraction"
    )
...
class TableExtractorConfig(BaseConfig):
    ...
    extract_format: Literal["markdown", "csv", "json", "html"] = Field(
        default="markdown", description="Output format for extracted tables"
    )
    preserve_formatting: bool = Field(
        default=True, description="Whether to preserve table formatting"
    )
    merge_cells: bool = Field(
        default=True, description="Whether to handle merged cells"
    )
    extract_headers: bool = Field(
        default=True, description="Whether to identify table headers"
    )
    min_rows: int = Field(default=2, ge=1, description="Minimum rows for valid table")
    min_columns: int = Field(
        default=2, ge=1, description="Minimum columns for valid table"
    )
    clean_data: bool = Field(
        default=True, description="Whether to clean extracted data"
    )
    include_caption: bool = Field(
        default=True, description="Whether to extract table captions"
    )
