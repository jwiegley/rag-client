...
def process_includes(yaml_dict: dict[str, Any], base_path: Path) -> dict[str, Any]:
    ...
    for key, value in yaml_dict.items():
        if isinstance(value, str) and value.startswith("!include "):
            ...
            result[key] = [
                process_includes(item, base_path) if isinstance(item, dict) else item
                for item in value
            ]
        else:
            result[key] = value
...
def validate_config(config: Config) -> None:
    ...
    if not config.retrieval.embedding:
        raise ValueError(
            "Configuration must include embedding settings in retrieval section"
        )

    if not config.retrieval.vector_store:
        raise ValueError(
            "Configuration must include vector_store settings in retrieval section"
        )
