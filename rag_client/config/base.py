...
class LLMBaseConfig(ModelConfig):
    ...
    context_window: int = Field(
        default=3900, gt=0, description="Maximum context window size"
    )
...
class ChunkingBaseConfig(BaseConfig):
    ...
    chunk_size: int = Field(
        default=1024, gt=0, description="Size of text chunks in characters"
    )
    chunk_overlap: int = Field(
        default=20, ge=0, description="Number of overlapping characters between chunks"
    )
