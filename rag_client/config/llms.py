...
class OpenAILLMConfig(APIConfig, LLMBaseConfig):
    ...
    model_name: str = Field(
        default=DEFAULT_OPENAI_MODEL, description="OpenAI model name"
    )
...
class PerplexityLLMConfig(APIConfig, LLMBaseConfig):
    ...
    model_name: str = Field(
        default="llama-3.1-sonar-small-128k-online", description="Perplexity model name"
    )
    temperature: float = Field(
        default=DEFAULT_TEMPERATURE, ge=0.0, le=2.0, description="Sampling temperature"
    )
    ...
    frequency_penalty: float = Field(
        default=1.0, ge=-2.0, le=2.0, description="Frequency penalty"
    )
...
class LMStudioLLMConfig(LLMBaseConfig):
    ...
    model_name: str = Field(..., description="LMStudio model identifier")
    base_url: HttpUrl = Field(
        default="http://localhost:1234/v1", description="LMStudio API base URL"
    )
    api_key: SecretStr = Field(
        default=SecretStr("lm-studio"), description="API key (usually 'lm-studio')"
    )
    temperature: float = Field(
        default=DEFAULT_TEMPERATURE, ge=0.0, le=2.0, description="Sampling temperature"
    )
...
class OpenRouterLLMConfig(APIConfig, LLMBaseConfig):
    ...
    model_name: str = Field(..., description="OpenRouter model identifier")
    temperature: float = Field(
        default=DEFAULT_TEMPERATURE, ge=0.0, le=2.0, description="Sampling temperature"
    )
    max_tokens: int | None = Field(
...
class TogetherAILLMConfig(APIConfig, LLMBaseConfig):
    ...
    model_name: str = Field(..., description="Together AI model identifier")
    temperature: float = Field(
        default=DEFAULT_TEMPERATURE, ge=0.0, le=2.0, description="Sampling temperature"
    )
    max_tokens: int | None = Field(
    ...
    streaming: bool = Field(default=False, description="Whether to stream responses")
    top_p: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Nucleus sampling parameter"
    )
...
class DeepSeekLLMConfig(APIConfig, LLMBaseConfig):
    ...
    model_name: str = Field(default="deepseek-chat", description="DeepSeek model name")
    temperature: float = Field(
        default=DEFAULT_TEMPERATURE, ge=0.0, le=2.0, description="Sampling temperature"
    )
    max_tokens: int | None = Field(
...
class AnthropicLLMConfig(APIConfig, LLMBaseConfig):
    ...
    max_tokens: int = Field(
        default=1024, gt=0, le=4096, description="Maximum tokens to generate"
    )
    context_window: int = Field(default=200000, gt=0, description="Context window size")
    streaming: bool = Field(default=False, description="Whether to stream responses")
    top_p: float = Field(
