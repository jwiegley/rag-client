...
class SentenceSplitterConfig(ChunkingBaseConfig):
    ...
    chunk_overlap: int = Field(
        default=200, ge=0, description="Number of overlapping characters between chunks"
    )
    separator: str = Field(
        default=" ", description="Separator to use between sentences"
    )
...
class CodeSplitterConfig(ChunkingBaseConfig):
    ...
    language: Literal[
        ...
        default="python", description="Programming language for syntax-aware splitting"
    )
    chunk_lines: int = Field(
        default=40, ge=10, le=500, description="Target number of lines per chunk"
    )
    chunk_lines_overlap: int = Field(
        default=15, ge=0, description="Number of overlapping lines between chunks"
    )
    max_chars: int = Field(
    ...
    def validate_lines_overlap(cls, v: int, info) -> int:
        ...
        if "chunk_lines" in info.data and v > info.data["chunk_lines"] // 2:
            raise ValueError(
                "chunk_lines_overlap should be at most half of chunk_lines"
            )
        return v
...
class MarkdownSplitterConfig(ChunkingBaseConfig):
    ...
    include_metadata: bool = Field(
        default=True, description="Whether to include markdown metadata in chunks"
    )
...
class HybridSplitterConfig(ChunkingBaseConfig):
    ...
    chunk_size: int = Field(
        default=1024, ge=100, le=4096, description="Target size for each chunk"
    )
    chunk_overlap: int = Field(default=200, ge=0, description="Overlap between chunks")
    primary_strategy: Literal["sentence", "semantic", "code", "markdown"] = Field(
        default="sentence", description="Primary splitting strategy"
    )
    fallback_strategy: Literal["sentence", "semantic", "code", "markdown"] = Field(
        default="sentence", description="Fallback strategy if primary fails"
    )
    ...
    min_chunk_size: int = Field(
        default=100, ge=10, description="Minimum size for a valid chunk"
    )
...
class RecursiveCharacterSplitterConfig(ChunkingBaseConfig):
    ...
    keep_separator: bool = Field(
        default=True, description="Whether to keep separators in chunks"
    )
...
class TokenSplitterConfig(ChunkingBaseConfig):
    ...
    add_special_tokens: bool = Field(
        default=True, description="Whether to add special tokens"
    )
