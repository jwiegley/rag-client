...
class TestChatCompletionsEndpoint:
    ...
    def test_chat_completion_with_temperature(
        self, client, auth_headers, mock_workflow
    ):
    ...
    def test_chat_completion_streaming(self, client, auth_headers, mock_workflow):
        ...
        with patch("rag_client.api.server.API_KEY", "test-api-key"):
            with patch("rag_client.api.server.rag_workflow", mock_workflow):
                mock_workflow.stream_chat = Mock(
        ...
        with patch("rag_client.api.server.API_KEY", "test-api-key"):
            with patch("rag_client.api.server.rag_workflow", mock_workflow):
                response = client.post(
                    "/v1/completions",
                    headers=auth_headers,
                    json={"model": "rag-model", "prompt": "Complete this"},
                )
                assert response.status_code == 200
                data = response.json()
    ...
    def test_completion_invalid_model(self, client, auth_headers):
        ...
        with patch("rag_client.api.server.API_KEY", "test-api-key"):
            response = client.post(
                "/v1/completions",
                headers=auth_headers,
                json={"model": "invalid-model", "prompt": "Complete this"},
            )
            assert response.status_code == 400
            assert "Model not found" in response.json()["detail"]
        ...
        with patch("rag_client.api.server.API_KEY", "test-api-key"):
            with patch("rag_client.api.server.rag_workflow", mock_workflow):
                response = client.post(
                    "/v1/embeddings",
                    headers=auth_headers,
                    json={"model": "rag-embedding", "input": "Test text"},
                )
                assert response.status_code == 200
                data = response.json()
        ...
        with patch("rag_client.api.server.API_KEY", "test-api-key"):
            with patch("rag_client.api.server.rag_workflow", mock_workflow):
                response = client.post(
                    "/v1/embeddings",
                    headers=auth_headers,
                    json={"model": "rag-embedding", "input": ["Text 1", "Text 2"]},
                )
                assert response.status_code == 200
                data = response.json()
